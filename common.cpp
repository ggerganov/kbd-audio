    ...
    bool readWaveform(std::ifstream &fin, TWaveformT<TSample> &res, int32_t offset, std::streamsize size) {
        if (std::is_same<TSample, TSampleI16>::value) {
            ...
            for (auto i = 0; i < (int) buf.size(); ++i)
                res[offset + i] = std::round(std::numeric_limits<TSampleI16>::max() * (buf[i] * iamax));
...
uint64_t t_ms() {
    return std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch()).count(); // duh ..
}
...
bool convert(const TWaveformT<TSampleSrc> &src, TWaveformT<TSampleDst> &dst) {
    ...
    for (auto i = 0; i < (int) src.size(); ++i)
        dst[i] = std::round(std::numeric_limits<TSampleDst>::max() * (src[i] * iamax));
...
template<typename TSampleInput, typename TSample>
bool
readFromFile(const std::string &fname, TWaveformT<TSample> &res, TTrainKeys &trainKeys, int32_t &bufferSize_frames) {
    trainKeys.clear();
...
template bool
readFromFile<TSampleF, TSampleI16>(const std::string &fname, TWaveformT<TSampleI16> &res, TTrainKeys &trainKeys,
            ...
            for (int i = ith; i < nPresses; i += nWorkers) {
                ...
                for (int j = 0; j < nPresses; ++j) {
                    ...
                    const auto ret = findBestCC(TWaveformViewT<T>{samples0 + pos0 + offsetFromPeak_samples - w, 2 * w},
                                                TWaveformViewT<T>{samples1 + pos1 + offsetFromPeak_samples - w - a,
                                                                  2 * w + 2 * a}, a);
...
template bool loadKeyPresses<TSampleI16>(const std::string &fname, const TWaveformViewT<TSampleI16> &waveform,
                                         TKeyPressCollectionT<TSampleI16> &keyPresses);
...
template bool
generateLowResWaveform<TSampleI16>(const TWaveformViewT<TSampleI16> &waveform, TWaveformT<TSampleI16> &waveformLowRes,
                                   int nWindow);
